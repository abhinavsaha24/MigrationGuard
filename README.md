...
## Guided Repair & Assistant Architecture

When a migration incompatibility is detected, MigrationGuard provides an end-to-end **Guided Repair System**:

```
Migration Failure → Evidence Context → Deterministic Repair Planner → Repair Proposal (Diff + Multi-Phase Rollout) → Authority Approval → CLI Local Application → Independent Re-Verification
```

### Core Trust Principles

1. **Deterministic Core as Scientific Ground Truth**: Compatibility verdicts (`PASS` / `FAIL`) and repair proposals are generated by deterministic algorithms.
2. **AI Provider Abstraction**: LLMs explain evidence and proposals in human language behind strict read-only, allowlisted tools with object-level authorization (IDOR prevention) and prompt injection sanitization. AI is strictly explanatory and can never alter a verdict or fabricate runtime evidence.
3. **Zero Silent Edits**: Neither `verify` nor `repair --show` touches project files. Modifications occur only with explicit user authority approval (`--approve` or `--yes`).
4. **Cryptographic Staleness Check**: Repair proposals are bound to the SHA-256 digest of the source schema. If files change before approval, application is safely rejected.
5. **Mandatory Post-Repair Re-Verification**: Applying a repair automatically triggers an independent 4-cell matrix execution to verify full compatibility.
